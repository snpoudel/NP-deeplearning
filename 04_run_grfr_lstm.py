...
def main(seed: int, device: str) -> None:
    ...
    loss_path = MODEL_DIR / f"grfr_lstm_seed{seed}_loss_curves.parquet"
    loss_df.to_parquet(loss_path, index=False)
    loss_df.to_parquet(MODEL_DIR / "grfr_lstm_loss_curves.parquet", index=False)
    ...
    timing_row = pd.DataFrame([{
        "model": "grfr_lstm", "seed": seed, "epochs_run": len(train_losses),
