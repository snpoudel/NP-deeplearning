...
def main(seed: int, device: str, mode: str = "dev") -> None:
    ...
    for gauge_id, gdf in gauge_dfs.items():
        ...
        ].dropna(subset=[TARGET, "qgrfr"]).copy().reset_index(drop=True)
        gdf_clean[TARGET] = gdf_clean[TARGET] - gdf_clean["qgrfr"]  # residual as target
    ...
    for gauge_id in sorted(val_per_gauge.keys()):
        ...
        phys_vals = gdf["qgrfr"].iloc[seq_len - 1:].values
