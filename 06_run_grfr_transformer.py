# Transformer post-processor for correcting GRFR streamflow outputs.
#
# Identical to 05_run_glofas_transformer.py but uses GRFR instead of GloFAS.
# Approach (residual correction):
#   1. Compute residual = qobs - qgrfr for every timestep
...
# Output path:    output/predictions/grfr_transformer/seed{seed}/nepal_{gauge_id}_grfr_transformer.parquet
...
def main(seed: int, device: str) -> None:
    ...
    model_path = MODEL_DIR / f"grfr_transformer_seed{seed}_best.pt"
    pred_dir = Path("output/predictions/grfr_transformer") / f"seed{seed}"
    pred_dir.mkdir(parents=True, exist_ok=True)

    # ------------------------------------------------------------------
    # 1. Load data and merge GRFR
    # ------------------------------------------------------------------
    print("Loading data...")
    ...
    n_before = len(all_df)
    all_df = all_df.dropna(subset=[TARGET, "qgrfr"]).reset_index(drop=True)
    n_dropped = n_before - len(all_df)
    if n_dropped > 0:
        print(f"  Dropped {n_dropped} rows with missing qobs or qgrfr")

    assert len(all_df) > 0, "No valid rows after filtering — check input data."

    all_df["residual"] = all_df[TARGET] - all_df["qgrfr"]
    ...
    # 6. Training loop
    # ------------------------------------------------------------------
    print("Training on residuals (qobs - qgrfr)...")
    ...
    loss_path = MODEL_DIR / f"grfr_transformer_seed{seed}_loss_curves.parquet"
    loss_df.to_parquet(loss_path, index=False)
    loss_df.to_parquet(MODEL_DIR / "grfr_transformer_loss_curves.parquet", index=False)
    ...
    timing_row = pd.DataFrame([{
        "model": "grfr_transformer", "seed": seed, "epochs_run": len(train_losses),
    ...
    qgrfr_val = val_df_orig["qgrfr"].iloc[seq_len - 1:].values
    qobs_val = val_df_orig["residual"].iloc[seq_len - 1:].values + qgrfr_val
    qsim_val = qgrfr_val + predicted_residuals_val

    metrics = compute_metrics(qobs_val, qsim_val)
    print(f"\nValidation metrics — final qsim (qgrfr + predicted residual):")
    ...
    for gauge_id, raw_df in gauge_dfs.items():
        ...
        g_df["qobs_orig"] = g_df[TARGET] + g_df["qgrfr"]
        g_df["residual"] = g_df[TARGET]
        ...
        qgrfr_out = g_df["qgrfr"].iloc[start:].reset_index(drop=True)
        qsim_out = qgrfr_out.values + predicted_residuals
        ...
        out_path = pred_dir / f"nepal_{gauge_id}_grfr_transformer.parquet"
        out_df.to_parquet(out_path, index=False)
        out_df.to_parquet(pred_dir.parent / f"nepal_{gauge_id}_grfr_transformer_mean.parquet", index=False)
