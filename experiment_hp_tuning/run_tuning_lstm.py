"""
experiment_hp_tuning/run_tuning_lstm.py
...
(shared/hyperparameters.py). Predictions are saved in the same format as
experiment_ae/run_ablation.py so evaluate_tuning.py can compare validation
loss across variants.

Prerequisite: run preprocessing/05_get_alphaearth_embeddings.py first to
produce input/alphaearth_embeddings.parquet.

Usage:
    python experiment_hp_tuning/run_tuning_lstm.py                  # production
    python experiment_hp_tuning/run_tuning_lstm.py --mode dev       # fast smoke-test
"""
...
from shared.models import LSTMModel
...
# ---------------------------------------------------------------------------
# Training / evaluation helpers (identical to experiment_ae/run_ablation.py)
# ---------------------------------------------------------------------------
...
def train_variant(
    ...
    print(f"\n{'='*60}")
    print(f"Variant: {variant_name}  |  hidden_size: {hidden_size}  |  seed: {SEED}{mode_tag}")
    print(f"{'='*60}")
    ...
    # Save loss curves (train_time_seconds repeated per row so evaluate_tuning.py
    # can read it alongside val_loss from the same file)
    pd.DataFrame({
...
def main(mode: str = "dev") -> None:
    ...
    # Load base gauge DataFrames and attach AlphaEarth embeddings
    print("\nLoading gauge data...")
    ...
    feature_cols = DYNAMIC_FEATURES + ae_cols

    for hidden_size in HIDDEN_SIZES:
        variant_name = f"lstm_h{hidden_size}"
        train_variant(variant_name, hidden_size, feature_cols, ae_gauge_dfs, device, mode=mode)

    print("\n=== All hidden sizes complete. Run experiment_hp_tuning/evaluate_tuning.py next. ===")


if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Hidden-size tuning — LSTM")
    parser.add_argument(
