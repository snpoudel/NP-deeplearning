"""
experiment_hp_tuning/run_tuning_transformer.py
...
Usage:
    python experiment_hp_tuning/run_tuning_transformer.py                  # production
    python experiment_hp_tuning/run_tuning_transformer.py --mode dev       # fast smoke-test
"""
...
from shared.models import TransformerModel
...
# ---------------------------------------------------------------------------
# Training / evaluation helpers (identical to experiment_ae/run_ablation_transformer.py)
# ---------------------------------------------------------------------------
...
def train_variant(
    ...
    print(f"\n{'='*60}")
    print(f"Variant: {variant_name}  |  d_model: {d_model}  |  seed: {SEED}{mode_tag}")
    print(f"{'='*60}")
...
def main(mode: str = "dev") -> None:
    ...
    for d_model in D_MODEL_SIZES:
        variant_name = f"transformer_h{d_model}"
        train_variant(variant_name, d_model, feature_cols, ae_gauge_dfs, device, mode=mode)

    print("\n=== All hidden sizes complete. Run experiment_hp_tuning/evaluate_tuning.py next. ===")


if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Hidden-size (d_model) tuning — Transformer")
    parser.add_argument(
