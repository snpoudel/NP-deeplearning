...
def main(seed: int, device: str) -> None:
    ...
    loss_path = MODEL_DIR / f"glofas_lstm_seed{seed}_loss_curves.parquet"
    loss_df.to_parquet(loss_path, index=False)
    loss_df.to_parquet(MODEL_DIR / "glofas_lstm_loss_curves.parquet", index=False)
    ...
    timing_row = pd.DataFrame([{
        "model": "glofas_lstm", "seed": seed, "epochs_run": len(train_losses),
    ...
    for gauge_id, raw_df in gauge_dfs.items():
        ...
        out_df.to_parquet(pred_dir.parent / f"nepal_{gauge_id}_glofas_lstm_mean.parquet", index=False)
