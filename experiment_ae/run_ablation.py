...
Trains three LSTM variants with seed=42 (single seed — sufficient for a
comparison experiment):

  A) lstm_dynamic      — 2 features: temperature + precipitation
  B) lstm_static       — 18 features: dynamic + existing 16 static attrs
  C) lstm_alphaearth   — 2 + emb_dim features: dynamic + AlphaEarth embeddings

Everything else (architecture, hyperparameters, train/val/test split) is
identical to 01_run_lstm.py.  Predictions are saved in the same format as
the main pipeline so evaluate_ablation.py can compare them directly.

Prerequisite: run preprocessing/05_get_alphaearth_embeddings.py first to
produce input/alphaearth_embeddings.parquet.

Usage:
    python experiment_ae/run_ablation.py           # production
    python experiment_ae/run_ablation.py --dev     # fast smoke-test
"""
...
def main(mode: str = "dev") -> None:
    ...
    variants = {
        ...
        "lstm_alphaearth": (DYNAMIC_FEATURES + ae_cols,    ae_gauge_dfs),
    }
